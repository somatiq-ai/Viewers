...
const handlePETImageMetadata = ({ SeriesInstanceUID, StudyInstanceUID }) => {
  ...
  try {
    ...
    // If we have no usable instance metadata, still attach safe defaults to avoid runtime errors
    if (!instanceMetadataArray.length) {
      imageIds.forEach(imageId => {
        metadataProvider.addCustomMetadata(imageId, 'scalingModule', { suvbw: { scaled: false } });
      });
      return;
    }
    ...
    imageIds.forEach((imageId, index) => {
      const factor = suvScalingFactors[index] ?? { suvbw: { scaled: false } };
      metadataProvider.addCustomMetadata(imageId, 'scalingModule', factor);
    });
  } catch (error) {
    // On any failure, attach safe defaults so downstream code that reads `.suvbw.scaled` won't crash
    imageIds.forEach(imageId => {
      metadataProvider.addCustomMetadata(imageId, 'scalingModule', { suvbw: { scaled: false } });
    });
    console.log(error);
